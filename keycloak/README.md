...
![Version](https://img.shields.io/badge/version-26.7.4-2E7D32)
...
Keycloak 26.7.4 configuravel em runtime para PostgreSQL ou Microsoft SQL Server. Health e metricas ficam habilitados por padrao na porta de gerenciamento `9000`.

## Referencia da imagem

| Item | Valor |
| --- | --- |
| Imagem | `lzocateli/keycloak:26.7.4` |
| Imagem base | `quay.io/keycloak/keycloak:26.7.4`, fixada pelo digest `sha256:82a778...e29b2c` para `linux/amd64` |
...
Verificacao realizada com a matriz oficial do Keycloak 26.7.4:
...
services:
  ...
  keycloak:
    image: lzocateli/keycloak:26.7.4
    command: ["start"]
...
O exemplo abaixo e experimental porque SQL Server 2025 nao consta na matriz suportada pelo Keycloak 26.7.4:
...
```bash
docker run --rm lzocateli/keycloak:26.7.4 --version
docker image inspect lzocateli/keycloak:26.7.4
```
...
| --- | --- | --- | --- |
| `26.7.4` | Imutavel | Keycloak 26.7.4 | Producao apos validar migrations e extensoes |
...
| Keycloak | 26.7.4 | Apache-2.0 | `https://github.com/keycloak/keycloak` |
| Imagem oficial | 26.7.4 | Apache-2.0 e licencas dos componentes | `https://quay.io/repository/keycloak/keycloak` |
...
- `26.7.4`: atualiza dependencias Java para corrigir vulnerabilidades detectadas pelo Trivy.
